...
export function loadConfig() {
  ...
  return {
    ...
    slippagePercent: parseNumber('SLIPPAGE_PERCENT', 15),
    gasPriceGwei: parseNumber('GAS_PRICE_GWEI', 0.05),
    tipRate: parseNumber('TIP_RATE', 0),
    takeProfitPercent: parseNumber('TAKE_PROFIT_PERCENT', 30),
    stopLossPercent: parseNumber('STOP_LOSS_PERCENT', 15),
    autoSellPercent: parseNumber('AUTO_SELL_PERCENT', 100),
    maxOpenPositions: parseNumber('MAX_OPEN_POSITIONS', 1),
    maxTaxPercent: parseNumber('MAX_TAX_PERCENT', 15),
    minProgressPercent: parseNumber('MIN_PROGRESS_PERCENT', 0),
    minLiquidityBnb: parseNumber('MIN_LIQUIDITY_BNB', 0.5),
    minFirstSwapBnb: parseNumber('MIN_FIRST_SWAP_BNB', 0),
